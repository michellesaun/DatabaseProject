...
--Procedure to insert a new record in the Members table
--This procedure will be used whenever a new member joins the club.
GO
CREATE PROCEDURE NewMember
	...
	@joinDate DATE,
	@phoneNumber VARCHAR(15),
	@memberAddress VARCHAR(155),
	@position VARCHAR(50)
AS
BEGIN
INSERT INTO Members(memberID, computingID, memberName, classYear, joinDate, phoneNumber, memberAddress, position)
VALUES (@memberID, @computingID, @memberName, @classYear, @joinDate, @phoneNumber, @memberAddress, @position);
END;

--Procedure to log a vote: 
--This procedure will be used whenever a club member votes in an election.
GO
CREATE PROCEDURE LogVote
	@electionID INT,
	@candidateID INT,
	@memberID INT
AS
BEGIN
	INSERT INTO Vote (electionID, candidateID, memberID) 
	VALUES (@electionID, @candidateID, @memberID);
END;

--Procedure to update advisor phone number:
--This procedure will be used whenever an advisor needs to update their current phone number. 
GO
CREATE PROCEDURE UpdateNumber
	@advisorID INT,
	@newNumber VARCHAR(15)
AS
BEGIN
	UPDATE Advisor
...
--Function to get all members running a given position:
GO
CREATE FUNCTION GetCandidatesByPosition
(
...
RETURN
(
	SELECT M.memberName FROM Members AS M
	JOIN Candidate AS C ON M.memberID = C.memberID
	WHERE M.position = @position_in
);

--Given a candidate, get the total number of votes for that candidate
GO
CREATE FUNCTION GetVotesForCandidate
(
	@candidateID_in INT
)
RETURNS INT
...
BEGIN
	...
	WHERE candidateID = @candidateID_in;

	RETURN @totalVotes;
END;

--Get the total amount a sponsor has donated, given the sponsor’s name
GO
CREATE FUNCTION GetAmountDonatedBySponsor
(
	@sponsorName_in VARCHAR(100)
)
RETURNS INT
AS
BEGIN
	DECLARE @totalDonated INT
	SELECT @totalDonated = SUM(amount) FROM Donation AS D
	JOIN Sponsor AS S ON D.sponsorID = S.sponsorID
	WHERE S.sponsorName = @sponsorName_in;

	RETURN @totalDonated;
END;

--View members in the club
GO
CREATE VIEW ViewMembers AS
(SELECT memberID, computingID, memberName, classYear, joinDate, position
FROM members);

--View Members Contact Information
GO
CREATE VIEW ViewContact AS
(SELECT memberID, memberName, phoneNumber, memberAddress
FROM members); 

--View Running Candidates Position 
GO
CREATE VIEW ViewCandidatePosition AS 
(SELECT memberID, memberName, position, joinDate
FROM members 
WHERE position IS NOT NULL); 

--Check that a member doesn’t vote more than once 
GO
CREATE TRIGGER trg_PreventMultipleVotesForMember
ON Vote AFTER INSERT
AS
BEGIN
	IF EXISTS (
		SELECT 1 FROM Vote
		WHERE electionID = (SELECT electionID FROM inserted)
		AND memberID = (SELECT memberID FROM inserted)
	)
		BEGIN 
			RAISERROR('This member has already voted in this election', 16, 1)
			ROLLBACK TRANSACTION
		END;
END; 

--Encrypt advisor’s phone number:
CREATE MASTER KEY ENCRYPTION BY PASSWORD = 'MasterKeyPassword148';
CREATE SYMMETRIC KEY PhoneNumberEncryptionKey 
WITH ALGORITHM = AES_128
ENCRYPTION BY PASSWORD = 'MasterKeyPassword148';

CREATE NONCLUSTERED INDEX idx_eventDate ON Event(eventDate);

CREATE NONCLUSTERED INDEX idx_ComputingID ON Members(computingID);

CREATE NONCLUSTERED INDEX idx_Position ON Election(position);
